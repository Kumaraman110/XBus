...
export async function main(): Promise<void> {
  ...
  // XBusError, or an fs error). A hook must NEVER block Claude, so a failure here
  // degrades: skip XBus this turn and exit 0. (Without this guard the throw escapes the
  // try below — which only wraps runCheckpoint — and crashes the hook.)
  let rootSecret: Buffer;
  try {
    rootSecret = loadOrCreateRootSecret(dataDir);
  } catch {
    process.exit(0); // XBus degraded this turn; Claude proceeds normally.
  }
