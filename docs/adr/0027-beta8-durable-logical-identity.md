...
- **RESOLVED in beta.12 (#315) — legacy-source data-safety + the "never mutated" invariant.**
  ...
  read-only and modify none of its AUTHORITATIVE bytes — the main DB file, the committed frames in
  its WAL, its ledger, secret, and ownership state are left byte-identical; a successful migration
  promotes only a verified-complete destination snapshot; a live-holder race fails closed; failure
  leaves the legacy installation independently usable.* (Caveat: a read-only open of a source with an
  uncheckpointed WAL may create/rewrite SQLite's ephemeral `-shm` wal-index and a 0-length `-wal`
  placeholder — derived files holding no committed data, regenerated by any reader; they are excluded
  from the immutability guarantee because their change cannot lose a row or render the source
  unusable. Immutable-mode, which would avoid even that, is intentionally not used: it would make the
  reader ignore the WAL and undercount rows.) (Original deferral, for history: "guarded by a post-copy
  hash compare + integrity_check that abort fail-closed … narrow torn-copy window" — accurate that it
  never lost data, but the source WAS mutated and the window was not the real risk.)
